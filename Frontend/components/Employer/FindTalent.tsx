...
const FindTalent = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.push(`/find-talent/talentlist?${queryParams.toString()}`);
  };
        ...
        <div className="max-w-full sm:max-w-4xl mx-auto border border-[#D9D9D9] p-2 sm:p-4 rounded-sm bg-white">
          <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-2 sm:gap-4">
            <div className="flex-1 relative">
              <Search className="absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 text-[#5A5A5A] h-5 w-5" />
              <input
                type="text"
            ...
            <div className="flex-1 relative">
              <MapPin className="absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 text-[#5A5A5A] h-5 w-5" />
              <input
                type="text"
