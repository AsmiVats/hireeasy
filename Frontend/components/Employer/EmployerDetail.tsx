...
interface JobPosting {
  ...
  totalViews: number;
  latestApplicationCount: number;

}

function EmployerDetailContent({ id, backToDashboard , isEditable = true}) {
    ...
    const fetchJobPosting = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/job/getJobPostings?employerId=${eid}&id=${id}`,
          {
            headers: {
        ...
        const finalData = data.job[0]
        console.log("Data to render", finalData)
        setJobPosting(finalData);
      } catch (error) {
        console.error('Error fetching job posting:', error);
        setError(error.message || 'Failed to load job posting');
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[#293E40]">{jobPosting?.companyName || 'Untitled'}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[#293E40]">{jobPosting?.noOfPeople}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[#293E40]">{jobPosting?.email}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[#293E40]">${jobPosting?.payRange.min} - ${jobPosting?.payRange.max} per week</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="rounded-lg border border-[#E6E6E6] bg-white p-6 sticky top-6">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[#293E40]">New Applicants - {jobPosting?.latestApplicationCount}</p>
              </div>
              
              <div className="flex items-center gap-2">
                <Users className="h-5 w-5 text-gray-400" />
                <p className="text-[#293E40]">Total Applicants - {jobPosting?.totalApplications}</p>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[#293E40]">{jobPosting?.alternativeEmail1 || "tony.stark@gmail.com"}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[#293E40]">{jobPosting?.alternativeEmail2 || "tony.stark@gmail.com"}</p>
                </div>
              </div>
              
              <div className="flex items-center gap-2">
                <Monitor className="h-5 w-5 text-gray-400" />
                <p className="text-[#293E40]">Total views - {jobPosting?.totalViews}</p>
              </div>
...
const EmployerDetail: React.FC<EmployerDetailProps> = ({ candidateId,backToDashboard, isEditable }) => {
  ...
  return (
    ...
    >
      <EmployerDetailContent id={id} backToDashboard={backToDashboard} isEditable={isEditable}/>
    </Suspense>
  );
};
