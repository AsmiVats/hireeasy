...
const SearchFilter = ({ criteria, onFilterChange, skillsResult }: SearchFilterProps) => {
  ...
  const handlePayScaleChange = (values: number[]) => {
    setPayRange(values);
    onFilterChange({
      minPay: values[0]*1000,
      maxPay: values[1]*1000,
    });
  };
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="space-y-4 pl-[10px]">
            <div className="flex justify-between text-sm text-[#009951]">
        ...
        <div>
          ...
          <div className="space-y-2 pl-[10px] h-48 overflow-y-scroll">
            {skillsResult.map((skill) => (
              <label
                key={skill}
                className="flex items-center gap-2 pb-[20px] text-sm text-[#293E40]"
              >
